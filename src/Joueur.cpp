...
void Joueur::premiereMain(Paquet *paquet) {
  for (int i = 0; i < 7; i++) {
    hand->addCarte(target->piocheCarte()); // ajouterCarte , drawCard
  }
}

// Afficher la main
void Joueur::afficherMain() {
  hand->viewerCartes(); // displayStack // affiche visuellement la main dans le terminal
}

// True si aucune carte dans la main
bool Joueur::verifieCartes() {
  if (hand->taillePaquet() == 0) { // deckSize
    return true;
  } else {
    return false;
  }
...
void Joueur::soustraireScore() {
  ...
  for (int i = 0; i < hand->taillePaquet(); i++) { // on parcourt le vecteur hand
    if (hand->getCartes().at(i).getValeur() <= 10) { // getCards // si carte inf à 10, pts=valeur de la carte
